...
class Node:
    ...
    def check(self, data, min, max):
        if data is None:
            return True
        elif data.data <= min or data.data >= max:
            return False

        return self.check(data.left, min, data.data) and self.check(data.right, data.data, max)

    def is_valid(self):
        """Is this tree a valid BST?"""
        return self.check(self, float('-inf'), float('inf'))
